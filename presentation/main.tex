...
\begin{frame}[shrink=8]{DDPM with DDIM Sampling}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      amplified by the long reverse chain. FID drops from 54.51 (no EMA)
      to 24.36 (with EMA) at 50 DDIM steps, a 55\% improvement.
    \end{column}
    \begin{column}{0.43\textwidth}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{block}{Fréchet Inception Distance (5\,000 samples)}
        ...
        \begin{tabular}{lcc}
          ...
          \textbf{DDPM+EMA} & \textbf{24.36}  & $\sim$12\,h \\
          \bottomrule
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{itemize}
        ...
        \item EMA weights reduce FID by 55\% vs.\ non-EMA at 50 steps
...
\begin{frame}{Generated Samples: Side by Side}
  \begin{figure}
    ...
    \begin{subfigure}[b]{0.31\textwidth}
      \includegraphics[width=\linewidth]{ddpm/samples_epoch0200.png}
      \caption{\textbf{DDPM}\\{\small FID 24.36}}
    \end{subfigure}
  \end{figure}
  \vspace{1pt}
  \small
...
\begin{frame}[shrink=8]{Latent Space Interpolation: Slerp vs.\ Linear}
  \centering
  \includegraphics[width=\textwidth,height=0.28\textheight,keepaspectratio]{dcgan/interpolation_compare.png}\\
  \small DCGAN --- \textbf{slerp} (top) vs.\ linear (bottom)\\[5pt]
  \includegraphics[width=\textwidth,height=0.28\textheight,keepaspectratio]{ddpm/interpolation_compare.png}\\
  \small DDPM --- \textbf{slerp} (top) vs.\ linear (bottom)\\[4pt]
  \footnotesize\color{mutedtext}
  Both methods shown. Linear interpolation (bottom rows) crosses a low-density
  region near the origin of $\mathcal{N}(0,I)$, producing blurrier intermediate
  frames --- most visible in DDPM (washed-out midpoints).
  Slerp preserves the norm of the latent vector and yields smoother transitions.
\end{frame}
...
\small
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{FID vs. DDIM steps (non-EMA)}
        ...
        \begin{tabular}{ccc}
          ...
          10  & 66.92 & $100\times$ \\
          25  & 60.02 & $40\times$ \\
          \textbf{50}  & \textbf{54.51} & $\mathbf{20\times}$ \\
          100 & 48.70 & $10\times$ \\
          200 & 39.81 & $5\times$ \\
          \bottomrule
        \end{tabular}\vspace{3pt}
    ...
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{EMA at 50 steps: $-55\%$ FID}
        \centering\vspace{3pt}
        \begin{tabular}{lc}
          \toprule
...
%
\begin{frame}[shrink=5]{Does Training on Dogs Help Cats?}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \textbf{Setup}
      \begin{itemize}
      ...
      \vspace{4pt}
      \small Combined FID (2\,500 cats + 2\,500 dogs) is more appropriate
      for a mixed-class model. 15-pt cat improvement is within
      seed variance ($\pm20$) --- suggestive, not conclusive.

      \vspace{4pt}
      \textbf{Qualitative:} samples show a mix --- some clearly cat-like
      (round face, pointed ears), some dog-like (longer snout), many
      ambiguous dark quadruped silhouettes.
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \begin{itemize}
        ...
        \item \textbf{WGAN-GP $n_c=2$}: pragmatic fix; theoretically
              $n_c\geq5$ required to preserve Lipschitz guarantee ---
              proper fix $\approx$500 ep., 200 ep.\ $\Rightarrow$ \textbf{15\,600 G-steps}
      \end{itemize}
    ...
    \begin{column}{0.45\textwidth}
      ...
      \centering\footnotesize Std.\ DCGAN (epoch 150)\\[4pt]
      \includegraphics[width=\linewidth]{dcgan_fixed_wgangp/samples_epoch0200.png}
      \centering\footnotesize WGAN-GP $n_c=2$ (epoch 200)
    \end{column}
  \end{columns}
\end{frame}
...
\footnotesize
  \begin{block}{RQ1: Architecture trade-off under 6\,GiB VRAM}
    DDPM: best quality (FID 24.36, 12\,h, 5.5\,GiB).
  ...
  \begin{block}{RQ4: DDIM speed-quality trade-off}
    50 steps: $20\times$ speedup, FID 54.51 (non-EMA) or 24.36 (with EMA).
    ...
    \begin{column}{0.52\textwidth}
      \begin{block}{Summary}
        ...
        \begin{tabular}{lcc}
          ...
          Model & FID & Strength \\
          \midrule
          \textbf{DDPM} & \textbf{24.36} & quality \\
      ...
      \vspace{5pt}
      \begin{itemize}
        \item Diffusion clearly outperforms GAN and VAE
        ...
        \item DCGAN bottleneck fixed: $7\times$ more G-updates
      \end{itemize}
    \end{column}
    \begin{column}{0.46\textwidth}
