...
\begin{frame}{Goal \& Motivation}
  \begin{columns}[T]
    \begin{column}{0.62\textwidth}
      ...
      and visual inspection of generated images.
    \end{column}
...
\begin{frame}[shrink=8]{DCGAN with WGAN-GP}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \vspace{5pt}
      \textbf{Result: FID 221.51} --- as shown later, the main bottleneck
      was an insufficient number of generator updates.
    \end{column}
...
\begin{frame}[shrink=8]{VQ-VAE with PixelCNN Prior}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \vspace{5pt}
      \textbf{Result: FID 187.54} --- autoencoder reconstructs well
      (MSE 0.0066), but PixelCNN prior limits sample quality.
      Early run with $K=1024$ had codebook collapse (30\% utilisation);
      fixed by switching to EMA updates and $K=512$.
    \end{column}
    \begin{column}{0.43\textwidth}
      \includegraphics[width=\linewidth]{vqvae/prior_samples.png}
      \centering\footnotesize PixelCNN prior samples (low visual quality)
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=8]{DDPM with DDIM Sampling}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \vspace{5pt}
      \textbf{Result: FID 24.11} (EMA weights, 50 DDIM steps) ---
      best result by a wide margin. EMA alone drops FID 57\%
      vs.\ non-EMA at the same step count.
    \end{column}
    \begin{column}{0.43\textwidth}
...
\begin{frame}{Generated Samples: Side by Side}
  ...
  VQ-VAE: structure largely indistinguishable \quad|\quad
  \textbf{DDPM: clearly recognisable cats}
\end{frame}
...
\begin{frame}[shrink=18]{Does Training on Dogs Help Cats?}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \vspace{3pt}
      \textbf{Qualitative:} visual quality poor --- most samples are
      indistinguishable blobs. Training FID worsened across epochs,
      suggesting the mixed dataset destabilised the generator.
    \end{column}
